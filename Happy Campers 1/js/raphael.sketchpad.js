 ...
 * 
 * Versions:
 * P.5.2 - Added scale setting, such that one can draw on a zoomed in object.
 * P.5.1 - Added mask, spotlight pen modes. Roswellstudios.com releases modifications to the public domain.
 *         required a minor hack to raphael-min.js. Find '"fill-opacity": 1,' insert '"fill-rule":"evenodd",'
 *         Snap.svg is not a drop-in replacement for Raphael
...
(function(Raphael) {
	...
	var Pen = function() {
		...
		rectangleSVG = function() {
			...
			var x1 = Math.min(pstart[0], pend[0]);
			var y1 = Math.min(pstart[1], pend[1]);
			...
			var x2 = Math.max(pstart[0], pend[0]);
			var y2 = Math.max(pstart[1], pend[1]);
			return "M" + x1 + "," + y1 +
				"L" + x1 + "," + y2 +
				"L" + x2 + "," + y2 + 
				"L" + x2 + "," + y1;
			...
			if (_c != null) {
				if (_points.length <= 1) {
					...
					if(_mode === 2) {
						...
						path.attr({ path: "M0,0L0,10000L10000,10000L10000,0Z " + 
						//Walk widdershins around the 4 points of the bounding rectangle
							rectangleSVG() +
							"Z"//close the loop
						});//will reset the path attr "path"
						path.attr({fill: '#000', "fill-rule":"evenodd","fill-opacity":"1"});
